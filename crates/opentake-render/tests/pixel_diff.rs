...
/// `p · M` (CG row-vector convention), mirroring `CGPoint.applying`.
fn apply(m: [f64; 6], x: f64, y: f64) -> (f64, f64) {
    (x * m[0] + y * m[2] + m[4], x * m[1] + y * m[3] + m[5])
}

/// PSNR (dB) over all RGBA bytes; `∞` when inputs are bit-identical.
...
/// Mean SSIM over 8×8 blocks (luma average of RGB). Identical frames → 1.0.
...
/// 64×64 texture split into four solid-color quadrants:
/// top-left red, top-right green, bottom-left blue, bottom-right yellow.
/// A single X or Y flip swaps the wrong pair of quadrants — a double flip
/// swaps both — so this is stricter than the two-band orientation check.
...
// ---------------------------------------------------------------------------
// Pure-function known-point checks (SPEC §6.3 step 1, §2.7). No GPU.
// ---------------------------------------------------------------------------

#[test]
fn affine_centered_half_canvas_matches_hand_computed() {
    // Upstream L599-604: topLeft=(0.25,0.25), sx=sy=0.5, tx=ty=25 (rs=100).
    ...
    // Source (0,0) -> canvas (25,25); source (100,100) -> (75,75).
...
fn affine_rotation_45_matches_compose_chain() {
    ...
    let rs = RenderSize::new(100, 100);
    let m = affine_transform(&t, (100.0, 100.0), rs);
    // placed = identity; then ∘ translate(-50,-50) ∘ rotate(45°) ∘ translate(50,50).
    ...
    // Center is invariant under pure rotation about itself.
...
fn affine_flip_horizontal_with_smaller_source() {
    // nat < render: sx = (rs/nat)*w*(-1). flip tx = (tl.x+w)*rs.w.
    let mut t = Transform::from_top_left(Point { x: 0.0, y: 0.0 }, 1.0, 1.0);
    t.flip_horizontal = true;
    let rs = RenderSize::new(100, 100);
    let m = affine_transform(&t, (200.0, 100.0), rs);
    // sx = (100/200)*1*(-1) = -0.5; tx = (0+1)*100 = 100; sy = 1; ty = 0.
    approx_affine(m, [-0.5, 0.0, 0.0, 1.0, 100.0, 0.0]);
    // Source (0,0) -> canvas (100,0); source (200,100) -> (0,100).
...
// ---------------------------------------------------------------------------
// GPU integration tests (SPEC §6.3 steps 2-7). Skip on no GPU.
// ---------------------------------------------------------------------------
...
fn quadrant_markers_land_in_authored_corners() {
    ...
    // Each quadrant center (16,16)/(48,16)/(16,48)/(48,48) — well inside its
    // block, away from linear-filtered edges.
...
fn quadrant_round_trip_psnr_is_high() {
    ...
    let frame = compositor
        .render_to_rgba(&dev.device, &dev.queue, RS, &fp, &mut resolver)
        .expect("render");
    // Same-size source→canvas quad with linear sampling: PSNR must clear the
    // SPEC §6.2 40 dB color bar by a wide margin (≥50 dB).
...
fn half_opacity_two_track_blend_matches_hand_computed() {
    ...
    // track0 (bottom): red @ opacity 0.5; track1 (top): green @ opacity 0.5.
    let mut tl = Timeline::new();
    tl.fps = 30;
    tl.width = Q as i32;
    tl.height = Q as i32;
    ...
    // Hand math (premult-over, 8-bit): red@0.5 over black -> [128,0,0,255];
    // then green@0.5 over that -> g=128, r=128*(1-128/255)≈64. So [~64,~128,0,255].
...
fn fade_envelope_smoothstep_endpoints_and_midpoint() {
    // Plan-level: opacity_at(start)=0, opacity_at(start+fade/2)=smoothstep(0.5)=0.5,
    // opacity_at(start+fade)=1. No GPU.
    ...
    // Linear interpolation would also give 0.5 at the midpoint, so assert the
    // smoothstep curve shape at a non-midpoint frame to lock the formula.
    let mut lin = full_canvas_clip("c1");
    lin.fade_in_frames = 10;
    lin.fade_in_interpolation = Interpolation::Linear;
    // frame 2 -> rel=2, t=0.2: smoothstep=0.04*2.6=0.104; linear=0.2.
...
fn fade_midframe_renders_half_brightness() {
    ...
    // White @ opacity 0.5 over black -> ~128.
    for &ch in &c[0..3] {
...
fn transform_keyframe_midframe_affine_equals_transform_at() {
    let mut clip = full_canvas_clip("c0");
    // position_track AnimPair = top-left in normalized canvas. Animate 0.1->0.5
    // over frames 0..10 (linear), so frame 5 top-left = (0.3, 0.3).
    ...
    // scale_track: width/height 1.0 -> 0.5 over 0..10, frame 5 = 0.75.
    ...
    // Plan-sampled affine must equal affine built from the domain-sampled
    // transform at the same frame (per-frame evaluation contract, §1.3).
    let expected = affine_transform(&clip.transform_at(5), (Q as f64, Q as f64), RS);
    approx_affine(fp.draws[0].affine, expected);
    // Endpoint sanity: frame 0 top-left (0.1,0.1) size 1.0 -> center (0.6,0.6).
    let t0 = clip.transform_at(0);
    approx(t0.center_x, 0.6);
    approx(t0.center_y, 0.6);
    approx(t0.width, 1.0);
    // Frame 10 top-left (0.5,0.5) size 0.5 -> center (0.75,0.75).
...
fn crop_keyframe_uv_varies_across_frames() {
    ...
    // Frame 0: full texture; frame 10: center half.
...
fn text_overlay_visible_above_video() {
    ...
    let plan = build_render_plan(&tl, RS, &Metrics);
    let fp = plan.frame(&tl, 0);
    // Video first, text on top (SPEC §4.2).
    assert_eq!(fp.draws.len(), 2);
    ...
    if rasterizer.has_fonts() {
        // With glyphs present, at least one pixel must be brighter in R than the
        // pure-blue video background (text contributes white pixels).
        let any_text = frame
            .rgba
            .chunks_exact(4)
...
fn ssim_identical_frames_score_near_one() {
    ...
    // Same input → deterministic output: SSIM must hit the SPEC §6.2 0.98 text
    // bar trivially (identical frames score 1.0).
    let s = ssim(&f1.rgba, &f2.rgba, f1.width, f1.height);
    assert!(s > 0.98, "SSIM of identical frames {s:.4} <= 0.98");
    // Sanity: a clearly different frame scores lower.
