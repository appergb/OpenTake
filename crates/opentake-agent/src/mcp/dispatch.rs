...
impl Dispatcher {
    ...
    fn set_clip_properties(
        ...
        if clip_ids.len() != 1 {
            return Err(ToolError::new(
                "set_clip_properties: partial transform currently supports exactly one clip per call",
            ));
        }
        let clip_id = clip_ids
            .into_iter()
            .next()
            .expect("clip_ids length checked above");
        ...
        properties.transform = Some(merge_transform_arg(clip.transform, transform_patch, aspect));
        let res = self.apply(EditCommand::SetClipProperties {
...
mod tests {
    ...
    #[test]
    fn set_clip_properties_partial_transform_multi_clip_is_rejected_without_mutation() {
        let h = seeded_transform_handle(
            Transform {
                width: 0.4,
                height: 0.2,
        ...
        assert!(r.is_error);
        assert!(
            r.text_joined().contains("partial transform"),
            "{}",
            r.text_joined()
        );
        let c = &h.timeline().tracks[0].clips[0];
