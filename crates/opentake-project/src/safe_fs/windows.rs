...
fn win32_query<T: Default>(
    ...
    {
        let error = last_win32(operation);
        #[cfg(test)]
        eprintln!("safe_fs win32_query class={class} status=error error={error:?}");
        return Err(error);
    }
    Ok(output)
}

fn remote_protocol_query_buffer(operation: SafeFsOperation) -> Result<FILE_REMOTE_PROTOCOL_INFO> {
...
fn query_entry_metadata(
    ...
    #[cfg(test)]
    eprintln!("safe_fs query_entry_metadata stage=FileAttributeTagInfo status=ok");
    let id: FILE_ID_INFO = win32_query(handle, FileIdInfo, operation)?;
    #[cfg(test)]
    eprintln!("safe_fs query_entry_metadata stage=FileIdInfo status=ok");
    let standard: FILE_STANDARD_INFO = win32_query(handle, FileStandardInfo, operation)?;
    #[cfg(test)]
    eprintln!("safe_fs query_entry_metadata stage=FileStandardInfo status=ok");
    ...
    #[cfg(test)]
    eprintln!("safe_fs query_entry_metadata stage=FileRemoteProtocolInfo status=start");
    ...
    #[cfg(test)]
    eprintln!(
        "safe_fs query_entry_metadata stage=FileRemoteProtocolInfo status=error win32={remote_error}"
    );
...
mod tests {
    ...
    fn remote_protocol_query_buffer_initializes_required_header() {
        ...
        assert_eq!(buffer.StructureVersion, 2);
        assert_eq!(
            buffer.StructureSize,
            u16::try_from(size_of::<FILE_REMOTE_PROTOCOL_INFO>()).unwrap()
        );
        assert_eq!(buffer.Protocol, 0);
        assert_eq!(buffer.ProtocolMajorVersion, 0);
        assert_eq!(buffer.ProtocolMinorVersion, 0);
        assert_eq!(buffer.ProtocolRevision, 0);
        assert_eq!(buffer.Reserved, 0);
        assert_eq!(buffer.Flags, 0);
        assert_eq!(buffer.GenericReserved.Reserved, [0; 8]);
        // SAFETY: the helper starts from a zeroed union, so its Reserved view is initialized.
        assert_eq!(unsafe { buffer.ProtocolSpecific.Reserved }, [0; 16]);
    }
