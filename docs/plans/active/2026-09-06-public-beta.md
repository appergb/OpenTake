...
- d1a020e 的 Motion 专项双平台 97+7 成功，模型资格双平台成功，完整 CI 8/9 通过。最后 Windows Tauri 单测暴露普通输出句柄没有 DELETE 权限，导致按句柄清理被拒绝。已统一必要权限、保留删除共享限制，输出验证通过 retained FD；75 项单测和 6 项真实导出集成本地通过、clippy通过。新增 Windows 输出专项资格，准备第五轮候选复验，未打 tag。
