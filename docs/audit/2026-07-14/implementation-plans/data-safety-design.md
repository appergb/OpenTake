...
- Exact files/symbols: `crates/opentake-project/src/bundle.rs#Project`, `crates/opentake-core/src/session.rs#EditorSession`, `crates/opentake-core/src/core.rs#AppCore::runtime_snapshot`, `crates/opentake-core/src/core.rs#AppCore::apply_at_revision`, `crates/opentake-domain/src/media.rs#MediaManifest`, `crates/opentake-project/src/gen_log.rs#GenerationLogEntry`, `crates/opentake-agent/src/mcp/core_handle.rs#AppCoreHandle::media_path`, `src-tauri/src/captions.rs#generate_captions`, `src-tauri/src/commands.rs#project_open_with_playback_and_prewarm`, `src-tauri/src/export.rs#export_video`, `src-tauri/src/mcp.rs#TauriMediaBridge`, `src-tauri/src/media.rs#MediaListDto::from_core_with_import_results`, `src-tauri/src/render.rs#composite_rgba`, `src-tauri/src/playback/commands.rs#PlaybackState`, `src-tauri/src/search.rs#resolve_assets`, `src-tauri/src/transcribe.rs#resolve_asset_from_snapshot`, `docs/specs/core/5-assembly.md`
...
- Test ownership:
  ...
  - `crates/opentake-core/src/core.rs#deferred_apply_rejects_version_and_project_drift_without_mutation` (reviewed-added): Deferred output cannot commit after its source revision changes.
  - `crates/opentake-agent/src/mcp/core_handle.rs#app_core_media_path_stress_never_mixes_project_snapshots` (reviewed-added): Concurrent stress exercises production Agent path resolution across project switches.
  - `src-tauri/src/captions.rs#caption_commit_rejects_stale_project_revision` (reviewed-added): Caption output is revision-bound at commit.
  - `src-tauri/src/commands.rs#project_open_mapped_boundaries_composite_acceptance` (reviewed-added): One fixture reaches every mapped post-open consumer.
  - `src-tauri/src/playback/commands.rs#prewarm_rejection_restores_active_playback_without_project_publish` (reviewed-added): Failed transition restores incumbent playback without publishing the prepared project.
  - `src-tauri/src/mcp.rs#transcript_batch_resolution_uses_one_snapshot_and_authoritative_types` (reviewed-added): The production batch resolver retains one snapshot and its authoritative media types.
...
- Acceptance criteria: Open a project by validating the persisted bundle/timeline/media/generation components and every synchronously fallible playback/prewarm transition admission before publishing editor state. Render, playback decoding, cache fill, and Agent/provider work are post-open consumers, so exercise them against the committed atomic runtime snapshot rather than making their lazy readiness a file-open precondition. A failed admission or malformed file must return a typed error/recovery result without silently replacing user data or partially publishing a session. Test valid, legacy, missing optional, malformed media.json, malformed generation log, dependency failure, save, replacement/close, and reopen with byte/semantic equality.
...
- Test ownership:
  ...
  - `src-tauri/src/mcp.rs#transcript_batch_resolution_uses_one_snapshot_and_authoritative_types` (reviewed-added): The production batch resolver retains one snapshot and its authoritative media types.
- Expected behavior: Project open assembles validated project/media/generation state without silent loss or placeholder dependencies.
- Acceptance criteria: Remove stale claims that project/render/playback/Agent are production `CoreDeps` fields. Use the explicit current interfaces: Project/EditorSession/AppCore for persistence, runtime snapshots plus playback/prewarm coordinators in src-tauri, and CoreHandle for Agent access. Deterministic owning-boundary tests must cover prepare failure, overlapping transition, prewarm rejection/cancellation with an incumbent playback resource, and successful epoch activation; core must never import Tauri/UI/provider concrete types. The legacy unsupported CoreDeps seam is not evidence for project-open coverage because production does not call it.
...
- Test ownership:
  ...
  - `src-tauri/src/mcp.rs#transcript_batch_resolution_uses_one_snapshot_and_authoritative_types` (reviewed-added): The production batch resolver retains one snapshot and its authoritative media types.
- Expected behavior: Project open assembles validated project/media/generation state without silent loss or placeholder dependencies.
- Acceptance criteria: Map each project/media/render/playback/Agent assembly point to its current implementation symbol and remove stale upstream-only or placeholder references. Exercise combined media/path reads through one AppCore runtime snapshot and Agent reads through the production CoreHandle rather than direct frontend or private Agent state mutation. Add a desktop contract test that opens one fixture and invokes media lookup, render-plan construction, playback media projection, Agent read/path resolution, save, and reopen through the mapped interfaces.
...
- Test ownership:
  ...
  - `src-tauri/src/mcp.rs#transcript_batch_resolution_uses_one_snapshot_and_authoritative_types` (reviewed-added): The production batch resolver retains one snapshot and its authoritative media types.
- Expected behavior: Project open assembles validated project/media/generation state without silent loss or placeholder dependencies.
- Acceptance criteria: Implement open ordering as validate bundle → load timeline/media/generation log → build the candidate session → admit playback/prewarm transition → publish snapshot/events → activate the new epoch → allow optional background work. On every pre-commit failure, drop prepared capabilities, restore any acquired transition, publish no usable session, and preserve all project files unchanged. Fault-inject the owned failure boundaries and assert cleanup/event order, then open/save/reopen valid and migrated fixtures with identical IDs, frames, media, and generation history.
