...
def build_fixture(root, label)
  ...
  run = {
    "id" => run_id.to_i, "run_attempt" => 1, "head_sha" => run_identity,
    "head_branch" => "audit/c1b-evidence-infra-20260717", "event" => "pull_request",
    "status" => "completed", "conclusion" => "success", "name" => "CI",
    "path" => ".github/workflows/ci.yml@refs/pull/1/merge",
    "pull_requests" => [{ "head" => { "sha" => expected } }],
  ...
  jobs = POLICY.fetch("receipts").each_with_index.map do |receipt, index|
    {
      ...
      "head_sha" => run_identity, "name" => "Safe filesystem (#{receipt.fetch('id')})",
  ...
  POLICY.fetch("receipts").each_with_index do |receipt_policy, index|
    ...
    receipt = {
      ...
      "event_name" => "pull_request", "run_id" => run_id, "run_attempt" => "1",
      ...
      "checked_out_sha" => expected, "commands" => commands, "aggregate_exit" => 0,
    }
    ...
    artifact = {
      ...
      "workflow_run" => { "id" => run_id.to_i, "head_sha" => run_identity },
    }
    artifacts << artifact
...
def run_validator(gate, expected, anchor, spec, implementation, fixture, fake_bin)
  env = { "PATH" => "#{fake_bin}#{File::PATH_SEPARATOR}#{ENV.fetch('PATH', '')}",
    "C1B_FAKE_GH_ROOT" => fixture }
